...
5. Before sending any lead, GET `https://goatara.com/api/save-lead` in an HTTP client or DevTools. Expect `405`, `Allow: POST` and **`X-Goatara-Lead-Relay: crm-v1`**. No form data is sent by this check. Load `https://goatara.com/js/main.js` with cache disabled and confirm `submission_id` and `saveLead`, not the old `saveLeadToSheets`. These prove the new server and browser code are active; they do not validate secret equality.
