...
- Subject, table template, honeypot, HTML validation, AJAX request and native POST fallback are unchanged.
- The existing success state and conversion calls still follow successful email AJAX delivery. A CRM failure cannot resend an email or prevent that success state.

For each valid submission, the browser also starts an independent, same-origin request:

```text
Partnership form
  -> FormSubmit email (existing AJAX + native fallback)
...
Starting the relay independently also covers the native email fallback. `keepalive` allows delivery to continue during navigation. A browser transport failure gets one retry with the identical request and submission ID. No lead payload is persisted in browser storage.
...
8. In browser DevTools, confirm the FormSubmit email request and `/api/save-lead` returning `200` with `ok: true` and the relay-version header. The browser must not call the CRM origin or contain any bearer/bypass secret. Verify one existing Lead conversion for successful AJAX delivery, without adding tracking.
...
Every response also supplies `X-Goatara-Delivery-Id`; use that value to find the matching website log. CRM `/api/workspace` and `/api/auth/me` GET logs only show page loading, not website delivery. A bare `{"ok":true}` relay response is the honeypot skip; acknowledged CRM deliveries include `submissionId`. A manually inserted CRM record or a zero New leads count does not establish whether a particular website request succeeded. Diagnose that request using its website response and matching delivery log.
...
Requires Node.js 24. `jsdom` is a development-only dependency for isolated tests of the real form script. No production package dependency was added:
...
The build is a syntax-validation gate for this static site and its serverless functions; Vercel performs function packaging. Tests stub CRM, Sheets, FormSubmit and conversion requests, do not load external page scripts, and must never send production leads or real emails. The suite covers all eleven fields, configuration errors, exact authentication/endpoint construction, awaited completion, safe logs, retries, legacy pages, validation, native email fallback and email-driven success during CRM failure. Local tests do not prove the deployed environment values or actual inbox receipt.
